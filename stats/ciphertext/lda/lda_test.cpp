...
int main() {
	...
	for (int d = 15; d < 16; d++) {
		cout << "d: " << d << " starts" << endl;
	        // Setting
        	uint32_t multDepth = 240;
	        uint32_t scaleModSize = 50;
        	uint32_t batchSize = 64;
		int n = 10; //The number of data
		int c = 2; //The number of classes
		int nd = max(n, d);
		std::vector<int> classes = {5, 5}; //The number of data per class; In this case, The number of data in first class is 2
		...
		for (int i = 0; i < c - 1; i++) { //Select u_i
	                ...
	                for (int j = i + 1; j < c; j++) { //Select u_j
                        	...
                        	for (int k = 0; k < d; k++) { //Select row
                                	Ciphertext<lbcrypto::DCRTPolyImpl<bigintdyn::mubintvec<bigintdyn::ubint<unsigned int> > > > delta = u_i - u_j;
        	                ...
        	                for (int k = 0; k < d - 1; k++) { //Select row
                	                for (int l = k + 1; l < d; l++) { //Select column
                        	                Ciphertext<lbcrypto::DCRTPolyImpl<bigintdyn::mubintvec<bigintdyn::ubint<unsigned int> > > > delta1 = u_i - u_j; //We need kth element
                                        	...
                                        	Ciphertext<lbcrypto::DCRTPolyImpl<bigintdyn::mubintvec<bigintdyn::ubint<unsigned int> > > > delta2 = u_i - u_j; //We need lth element

	                                        Ciphertext<lbcrypto::DCRTPolyImpl<bigintdyn::mubintvec<bigintdyn::ubint<unsigned int> > > > increment = cc -> EvalMult(delta1_rotate, delta2); //lth element is increment of A[k][l]
		...
		vector<Ciphertext<lbcrypto::DCRTPolyImpl<bigintdyn::mubintvec<bigintdyn::ubint<unsigned int> > > >> B_inverse = matrix_inverse(B, d, 10.0, cc); //The transpose of B_inverse
