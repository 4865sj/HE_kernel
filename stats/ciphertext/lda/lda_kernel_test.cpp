...
int main() {
	...
	for (int d = 3; d < 13; d++) {
		cout << "d: " << d << " starts" << endl;
        	// Setting
	        uint32_t multDepth = 100;
        	uint32_t scaleModSize = 50;
	        uint32_t batchSize = 64;
	        int n = 9; //The number of data
		int nd = max(n, d);
	        int c = 3; //The number of classes
        	std::vector<int> classes = {3, 3, 3}; //The number of data per class; In this case, The number of data in first class is 3
		...
		vector<Ciphertext<lbcrypto::DCRTPolyImpl<bigintdyn::mubintvec<bigintdyn::ubint<unsigned int> > > >> N_inverse = matrix_inverse(N, n, 10.0, cc); //The transpose of N_inverse
