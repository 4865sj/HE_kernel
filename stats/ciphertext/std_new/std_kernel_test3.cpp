...
int main() {
	...
	for (int d = 1; d < 2; d++) {
		...
		for (int i = 0; i < n; i++) {//Select data_i
			...
			for (int j = 0; j < n; j++) {//Select data_j
				...
				for (int k = 0; k < d; k++) {//Select dimension
					...
					kernel_k[i][j] = cc -> EvalAdd(increment_k, kernel_k[i][j]); //i,j-th element of kernel_k is k-th element of kernel_k[i][j]
		...
		for (int i = 0; i < d; i++) {//Select dimension
			...
			} //The i-th element of sum_diag_kernel is the summation of diagonal elements
			sum_diag_kernel = cc -> EvalMult(sum_diag_kernel, one_n_plaintext[i]); //Eliminate except the i-th element
		...
		for (int i = 0; i < d; i++) {//Select dimension
			...
			} //The i-th element of sum_all_kernel is summation of all elements	
			sum_all_kernel = cc -> EvalMult(sum_all_kernel, one_n_plaintext[i]); //Eliminate except the i-th element
