...
int main() {
	...
	std::vector<Ciphertext<lbcrypto::DCRTPolyImpl<bigintdyn::mubintvec<bigintdyn::ubint<unsigned int> > > >> dataset = {c1, c2};
	
	int rows = 2; //The number of rows of kernel
	int cols = 2; //The number of columns of kernel
	Ciphertext<lbcrypto::DCRTPolyImpl<bigintdyn::mubintvec<bigintdyn::ubint<unsigned int> > > > val = c1; //Temprory data
