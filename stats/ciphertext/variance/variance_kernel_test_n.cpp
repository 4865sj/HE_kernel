...
int main() {
	...
	int d = 15; //Dimension
        ...
        //test 10 case
        for (int n = 40; n < 80; n = n + 10) { //dimension
			...
			Ciphertext<lbcrypto::DCRTPolyImpl<bigintdyn::mubintvec<bigintdyn::ubint<unsigned int> > > > second_term = ct_zero_n; //Initial value is zero

			for (int i = 0; i < n; i++) {
				Ciphertext<lbcrypto::DCRTPolyImpl<bigintdyn::mubintvec<bigintdyn::ubint<unsigned int> > > > increment = kernel[i];
				Ciphertext<lbcrypto::DCRTPolyImpl<bigintdyn::mubintvec<bigintdyn::ubint<unsigned int> > > > kernel_rotate = kernel[i];
				for (int j = 1; j < n; j++) {
					kernel_rotate = cc -> EvalRotate(kernel_rotate, 1);
					increment = cc -> EvalAdd(increment, kernel_rotate);
				} //The first element of increment is K(x_i, x0) + K(x_i, x1) + ...
