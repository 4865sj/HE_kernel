...
int main() {
	...
	for (int d = 5; d < 16; d++) {
		...
		auto mean_square_vector = ct_zero_n; //Initial value
		for (int i = 0; i < d; i++) {//Select kernel
			auto sum_all_kernel = ct_zero_n;
			...
			auto increment = cc -> EvalAdd(sum_all_kernel, a*a); //Multiply 1/n^2
			increment = cc -> EvalMult(increment, one_d_plaintext[i]); //Eliminate except i-coordinate
			mean_square_vector = cc -> EvalAdd(mean_square_vector, increment); //Update i-coordinate
		}

		
		auto mean_vector = sqrt(mean_square_vector, cc);
		...
		//Decryption & Decoding
		Plaintext result;
		cc -> Decrypt(keys.secretKey, mean_vector, &result);
		result -> SetLength(d);
