...
int main() {
	...
	float time4 = -clock();
	Ciphertext<lbcrypto::DCRTPolyImpl<bigintdyn::mubintvec<bigintdyn::ubint<unsigned int> > > > distance_square = cc -> EvalAdd(kernel[0][0], kernel[1][1]); //K(x1, x1) + K(x2, x2)
	distance_square = cc -> EvalSub(distance_square, kernel[0][1]); //K(x1, x1) + K(x2, x2) - K(x1, x2)
	distance_square = cc -> EvalSub(distance_square, kernel[0][1]); //K(x1, x1) + K(x2, x2) - 2K(x1, x2)
